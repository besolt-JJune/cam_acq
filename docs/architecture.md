...
```
RAM Ring Buffer (Bayer 4K, pre + active window)
...
녹화 window는 **호스트 monotonic** 기준 (전 채널 동시). PTP 없음.

#### 3.2.1 Trigger 종료 조건 (`RecordingTrigger`)

`RECORDING_BUFFER_SEC` = 5초 예시.

| 모드 | 시작 | 종료 | 파일명 |
|------|------|------|--------|
| **Event (auto)** | person 최초 검출 | 마지막 person 검출 후 **연속 5초 무검출** (재검출 시 카운트다운 리셋) | `{ts}_cam{N}_seg{MM}.mp4` |
| **Manual** | `POST /api/recording/trigger` (또는 Dashboard **● REC**) | `POST /api/recording/stop` (**■ STOP**) | `{ts}_cam{N}_seg{MM}_manual.mp4` |

**우선순위:** manual > event. 수동 녹화 중 event 검출은 무시; event 중 수동 시작 시 manual이 대체.

**Event 예시 (buffer=5s)**

1. detect → no detect → 5초 후 종료
2. detect → no detect → (침묵 3초) → 재검출 → no detect → 재검출 시점 + 5초 후 종료

Ring **pre** 구간: encode window 시작 시 `started_at − RECORDING_BUFFER_SEC`.  
`ended_at`에는 event 침묵 tail(또는 manual stop 시각)이 포함되며, 별도 post-buffer 가산은 하지 않는다.

구현: `detection/events.py` (`TriggerAction`), `recording/controller.py` (`apply_trigger_action`).
...
| GigE disconnect (요구) | dashboard: per-cam 재연결·스트림 복구 — `13_gige_disconnect_recovery.md` §3 |
| Recording on disconnect (요구) | 해당 cam 세그먼트 즉시 close; reconnect 시 새 파일로 재개 — §13 §4 |
| UI | Phase 5 별도 설정 창 (`10_monitoring_design.md` §3.1) — **구현** (`--with-monitoring`) |
...
HostMetricsSampler @ SYSTEM_METRICS_POLL_SEC
  ...
  → footer: recording state + manual elapsed (`manual_elapsed_sec`)
```

**통합 모드 녹화:** `--with-monitoring`이면 pre-buffer·수동 REC 항상 활성 (`need_recording`). YOLO 자동 녹화는 `--no-event-recording` 없을 때만.
