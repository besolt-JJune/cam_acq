...
| Phase 2.3 `--recovery` (코드) | ⚠️ grab 전용 — **yolo-live·recording 통합 미구현** (`13_gige_disconnect_recovery.md`) |
...
**스펙:** dashboard 재연결 + recording disconnect split/resume — `13_gige_disconnect_recovery.md`  
**현재:** `grab_healthcheck --recovery`만 offline callback·IP 재연결 검증. yolo-live는 미연동.

**전제:** 통합 구현 후, `cam-acq-yolo-live --with-monitoring` 실행 중 **1회** Ethernet 분리·재연결
...
# (구현 후) 운영 E2E — yolo-live + event recording
./scripts/yolo_live_dashboard.sh
# 케이블 분리 5~10s → 재연결; API·STORAGE_PATH 세그먼트 확인 (§13 §3.4·§4.6)
```
...
**기대 (yolo-live E2E, 구현 후):** `13_gige_disconnect_recovery.md` §3.4·§4.6
