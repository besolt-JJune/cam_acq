...
**미완:** 3ch 코덱 부하 재측정(R2), 1h YOLO soak — `15_3ch_resource_profiling.md`.
...
3ch 추정 (선형, buffer 5s): ring ≈ **12.0 GB**, RSS ≈ **16 GB**.

### 5.1.1 Phase 4.9 실측 (2026-06-30, 3ch, buffer 2s)

`cam-acq-memory-profile` 60s — 상세: **`15_3ch_resource_profiling.md` §2**.

| 항목 | cam0 | cam1 | cam2 | 합계 / peak |
|------|------|------|------|-------------|
| ring (measured) | 1.65 GB | 1.65 GB | 1.65 GB | **5.24 GB** |
| process RSS peak | — | — | — | **7.90 GB** |
| system RAM peak | — | — | — | **11.9 GB** (38.5%) |
| VRAM peak (encode) | — | — | — | **2815 MB** |

`RECORDING_BUFFER_SEC=2` 유지 시 32 GB RAM·16 GB VRAM 내 **T12 PASS**. ring overflow는 녹화 중 grab>encode 이슈 (`15_3ch_resource_profiling.md` §2.5).

### 5.2 추정식

```
capacity = int(fps × RECORDING_BUFFER_SEC × 3) + 5   # recording/buffer.py
1 frame (Bayer8) = width × height
ring_total = capacity × frame_bytes × num_cameras
RGB ring     ≈ ring_total × 3
NV12 ring    ≈ ring_total × 1.5   (참고; 미구현)
```

### 5.3 Ring 용량 비교표 (내부 RAM, 추정)

해상도 **4024×3036** (Phase 4.9 실측). GigE·디스크 제외.

**운영 파라미터**

| 시나리오 | fps | `RECORDING_BUFFER_SEC` | frames/ch | vs 기준 |
|----------|-----|------------------------|-----------|---------|
| **A — 실측 기준** | 23 | 5 | 350 | 100% |
| **B — 단축안** | 20 | 2 | 125 | **35.7%** |

프레임 ≈ **11.66 MiB** (Bayer8).

**Ring only (Bayer)**

| 시나리오 | 2ch | 3ch |
|----------|-----|-----|
| A (23 / 5s) | 8.0 GB (실측) | 12.0 GB (추정) |
| B (20 / 2s) | **2.8 GB** | **4.3 GB** |

**Ring only (RGB888 — encode debayer 생략 가정, 미구현)**
...
**RSS 추정 (ring + 프로세스; 2ch 실측 α≈2.8 GB에서 선형 보간)**

| 시나리오 | ch | ring (Bayer) | RSS 추정 | 32 GB RAM |
|----------|-----|--------------|----------|-----------|
| A | 2ch | 8.0 GB | 10.8 GB (실측) | 여유 |
| A | 3ch | 12.0 GB | ~16 GB (추정) | 수용 가능 (YOLO·OS 별도) |
| B | 3ch | 4.3 GB (추정) | **5.24 GB / 7.9 GB RSS** (실측 R1) | 여유 증가 |
| B | 3ch RGB ring | 12.8 GB | **~16–18 GB** (추정) | 미구현 |

단축안 B는 pre/post **2초**만 보존 (기능 트레이드오프). 채택 시 `.env`·`AcquisitionFrameRate`·healthcheck fps 기준 정합 필요.

Bayer ring 유지 + encode 시 debayer가 **현재 구현**; RGB ring은 RAM 절감 대신 **연산 중복**을 줄이는 별도 설계안이며 **미구현**.
