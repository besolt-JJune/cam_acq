...
| 항목 | 지금 | 추후 |
|------|------|------|
| 3ch YOLO debayer | ✅ **전 채널 `gpu_phase3`** — ~20fps/ch (2026-06-30, `yolo_live_3ch_gpu.json`) | 1h soak 시 fps·overflow 모니터 |
| GPU 2ch + CPU 1ch 혼합 live debayer | **채택 안 함** | fps 미달 시에만 재검토 (`§4`) |
...
| 미완 | 3ch 1h YOLO+NVENC soak, `cam-acq-memory-profile` 3ch |
...
### 2026-06-30 — 3ch YOLO live (`gpu_phase3`)
...
| cam | ip | fps_pushed_avg | 결과 |
|-----|-----|----------------|------|
| 0 | 10.10.1.3 | 20.04 | PASS |
| 1 | 10.10.4.3 | 20.04 | PASS |
| 2 | 10.10.3.3 | 20.04 | PASS |

`status: PASS`, engine `yolov8m_person_b3_gpu0_fp16.engine`.  
기준 `fps_pushed ≥ 23×0.8` (18.4) 충족 → **혼합 debayer(§4) 불필요**.

3ch event·manual 녹화: `20260630_152223_cam{0,1,2}_seg0000.mp4`, `152217_*_manual.mp4`.
